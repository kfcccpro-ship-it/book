      ...
      try {
        ...
        window.addSubBook = async function addSubBookAtomic() {
          ...
          try {
            ...
            console.error(e);
            showNotification(`부교재 추가 실패: ${e.message || e}`, 'error');
          }
        };
        ...
        window.deleteSubBook = async function deleteSubBookAtomic(id, name) {
          ...
          try {
            ...
            console.error(e);
            showNotification(`부교재 삭제 실패: ${e.message || e}`, 'error');
          }
        };
        ...
        window.submitSubBookAction = async function submitSubBookActionAtomic() {
          ...
          const released = Number(book.released_quantity || 0);
          const rem = stock - released;
          if (actionType === '출고' && qty > rem) { showNotification(`잔여 재고(${rem}권)를 초과할 수 없습니다.`, 'error'); return; }
          if (!confirm(`📗 부교재 ${actionType} 확인\n\n${book.book_name}\n날짜: ${date}\n수량: ${qty}권\n${notes ? '메모: '+notes : ''}\n\n진행하시겠습니까?`)) return;

          try {
            const newStock = actionType === '입고' ? stock + qty : stock;
            const newReleased = actionType === '출고' ? released + qty : released;
            await subBookService.updateSubBookWithLog({
              subBookId: id,
              expectedStock: stock,
              expectedReleased: released,
              updates: { stock_quantity: newStock, released_quantity: newReleased },
              log: {
                action_type: actionType,
                quantity: qty,
                action_date: new Date(date + 'T09:00:00').toISOString(),
                notes: notes || `${actionType} ${qty}권`
              },
              actor: currentUser
            });
            showNotification(`부교재 ${actionType} 완료! ${book.book_name} ${qty}권`, 'success');
            closeSubBookActionModal();
            const groupKey = document.getElementById('sub-book-group-key')?.value || book.course_group_key;
            await refreshSubBooks(groupKey);
          } catch(e) {
            console.error(e);
            showNotification(`부교재 ${actionType} 실패: ${e.message || e}`, 'error');
          }
        };
        ...
        window.submitQuickRelease = async function submitQuickReleaseAtomic() {
          ...
          const totalReleased = getGroupTotalReleased(groupKey);
          const remaining = totalStock - totalReleased;
          if (qty > remaining) { showNotification(`잔여 재고(${remaining}권)를 초과할 수 없습니다.`, 'error'); return; }
          const target = courses
            .filter(c => getGroupKey(c.course_name) === groupKey && ['입고완료','출고완료'].includes(c.status))
            .sort((a,b) => new Date(a.start_date)-new Date(b.start_date))[0];
          if (!target) { showNotification('입고완료 또는 출고완료 상태의 과정이 없습니다.', 'error'); return; }
          if (!confirm(`📤 급 출고 확인\n\n${displayName}\n출고 수량: ${qty}권\n사유: ${notes}\n\n진행하시겠습니까?`)) return;
          try {
            const prevReleased = Number(target.released_quantity || 0);
            const newReleased = prevReleased + qty;
            ...
            await writeService.updateCourseWithLog({
              ...
              log: { course_name:target.course_name, action_type:'출고', action_date:now, quantity:qty,
                previous_status:target.status, new_status:'출고완료',
                notes:`급 출고 ${qty}권 / 사유: ${notes} / 잔여 ${remaining - qty}권` }
            });
            showNotification(`급 출고 완료! ${qty}권 → ${displayName} 잔여 ${remaining - qty}권`, 'success');
            document.getElementById('quick-release-quantity').value = '';
            document.getElementById('quick-release-notes').value = '';
            document.getElementById('quick-release-remain').textContent = '';
            await loadCourses();
            await loadDashboardData();
            if (typeof renderStockLists === 'function') renderStockLists();
          } catch(e) {
            console.error(e);
            showNotification(`급 출고 실패: ${e.message || e}`, 'error');
          }
        };
        ...
        window.processImport = async function processImportGuarded() {
          ...
          for (let i=1;i<lines.length;i++) {
            ...
            if (fields.length < 4) { showNotification(`${i+1}행 형식이 올바르지 않습니다. 가져오기를 중단했습니다.`, 'error'); return; }
            const [courseName,startDate,endDate,studentCount,releaseDate,classroom] = fields;
            const start = new Date(startDate);
            if (!courseName || Number.isNaN(start.getTime()) || !(parseInt(studentCount) > 0)) {
              showNotification(`${i+1}행 과정명/시작일/인원을 확인해주세요. 가져오기를 중단했습니다.`, 'error'); return;
            }
          ...
          try {
            for (const row of rows) {
              const now = new Date().toISOString();
              const newId = 'import_' + Date.now().toString(36) + '_' + Math.random().toString(36).slice(2,8);
              const course = {
                id:newId, course_name:row.courseName,
                start_date:new Date(row.startDate).toISOString(),
                end_date:row.endDate ? new Date(row.endDate).toISOString() : null,
                student_count:row.studentCount, stock_quantity:0, released_quantity:0,
                scheduled_release_date:new Date(row.releaseDate).toISOString(),
                classroom:row.classroom || '', status:'입고대기',
                created_by:currentUser.name, updated_by:currentUser.name,
                created_at:now, updated_at:now
              };
              await writeService.insertCourseWithLog({ course, actor:currentUser,
                log:{ course_name:row.courseName, action_type:'추가', action_date:now,
                  previous_status:null, new_status:'입고대기', notes:`가져오기 과정 추가: ${row.courseName}` } });
            }
            showNotification(`가져오기 완료: ${rows.length}건`, 'success');
            closeImportModal(); await loadCourses(); await loadDashboardData();
          } catch(e) {
            console.error(e);
            showNotification(`가져오기 중단: ${e.message || e}. 이미 완료된 행은 작업로그에서 확인할 수 있습니다.`, 'error');
          }
        };

        window.openResetModal = function disabledResetModal() {
          showNotification('Firebase 운영 버전에서는 전체 데이터 초기화 기능을 사용하지 않습니다. 개별 수정/삭제 기능을 이용해주세요.', 'warning');
        };
        window.confirmReset = function disabledConfirmReset() {
          showNotification('전체 데이터 초기화는 안전을 위해 비활성화되어 있습니다.', 'warning');
        };
        window.resetAllData = async function disabledResetAllData() {
          throw new Error('Firebase 운영 버전에서는 전체 데이터 초기화가 비활성화되어 있습니다.');
        };

        window.__mgAtomicPatchWave4Ready = true;
        window.__mgAtomicPatchVersion = 'wave4';
        console.info('[Firebase Migration] Atomic patch wave4 enabled: sub-books, quick release, guarded import; full reset disabled');
      } catch(e) {
        console.error('[Firebase Migration] Atomic patch wave4 init failed', e);
        window.__mgAtomicPatchWave4Ready = false;
      }
    }, 0);
  });
})();
